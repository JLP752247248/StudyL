...
public class Tasks {
	...
	void downloadTask(){
		...
		List<Map<String, Object>> mapL=dao.queryForList("select * from vedioinfo where state=0 limit "+(maxQueueSize-waitSize));
	...
	class DownThread implements Runnable{
		...
		public void run() {
			...
			dao.update("update vedioinfo set state= 2 where url='"+url+"'");
			int res=RestClient.downloadFile(url, aimFile);
			if(res<0) {
				logger.info("[{}],[{}]download failed!!!",aimFile,url);
				dao.update("update vedioinfo set state= -1 where url='"+url+"'");
				try {
